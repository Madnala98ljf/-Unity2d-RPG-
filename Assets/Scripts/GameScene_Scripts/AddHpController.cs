...
public class AddHpController : MonoBehaviour {
    public int add_hp;
    ...
    void Update () {
        if (pause.isPause == false) {
            if (!CD_Trigger) { // 如果技能不再冷却中，则释放技能并开始冷却计时；如果技能在冷却，则跳过；
                if (Input.GetKeyDown (s)) {
                    if (gameController.hp != hp_max) {
                        audio.Play();
            ...
            if (CD_Trigger) {
                ...
                if (m_CD_Left < 0) { // 如果剩余冷却时间为0，则停止冷却，并重新初始化相关变量。
